        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                    <i class="fas fa-info-circle mr-2 text-primary"></i> About Headers
                </h1>
                <p class="text-sm text-gray-500 mt-1">Manage the introduction titles, badges, and status for the About Us
                    section.</p>
            </div>
                        ...
                        @forelse($headers as $header)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-semibold rounded border border-gray-200 inline-flex items-center">
                                        @if ($header->badge_icon)
                                            <i class="{{ $header->badge_icon }} mr-1.5 text-primary/70"></i>
                                        @endif
                                        {{ $header->badge_text }}
                                    </span>
                                </td>
                                ...
                                {{-- Title --}}
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900">{{ Str::limit($header->title, 50) }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex justify-center items-center space-x-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('about-header.edit', $header) }}"
                                        ...
                                        {{-- Delete --}}
                                        {{-- <form action="{{ route('about-header.destroy', $header) }}" method="POST"
