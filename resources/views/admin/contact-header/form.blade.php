        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div class="bg-primary px-8 py-6">
                <h1 class="text-2xl font-bold text-white tracking-tight flex items-center">
                    <i class="fas fa-envelope-open-text mr-3 opacity-80"></i>
                    {{ $header ? 'Edit Contact Header' : 'Create New Contact Header' }}
                </h1>
                <p class="text-indigo-100/80 text-sm mt-1">Update the main heading and supporting text for your contact page
                    to engage visitors.</p>
            </div>
            ...
            <div class="p-8">
                ...
                <form method="POST"
                    action="{{ $header ? route('contact-header.update', $header) : route('contact-header.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($header)
                        @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="title" placeholder="e.g. Get In Touch With Us"
                            value="{{ old('title', $header->title ?? '') }}"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-all outline-none font-bold text-lg">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" placeholder="Provide a brief introductory sentence for the contact page..."
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary focus:border-primary transition-all outline-none leading-relaxed">{{ old('description', $header->description ?? '') }}</textarea>
                    </div>
